def main():
    ...
    EfficientB1  = models.efficientnet_b1(pretrained=True).to(device)
    ...
    set_parameter_requires_grad(EfficientB1,feature_extracting=True)
    ...
    def train_model(model, dataloaders, criterion, optimizer, num_epochs=epochs1):
      for epoch in range(num_epochs):
          ...
          for i,(X,y) in enumerate(CTTrain):
              ...
              CTActuals.append(y)
              CTPredictions.append(outputs)
              loss.backward()
              optimizer.step()
              #break;
                
           Epoch_Loss = Loss/(len(CTTrain))
    ...
    #finetuning
    truth_parameter_requires_grad(EfficientB1)
    cnt1 =0
    cnt2 =0
    for (p) in EfficientB1.parameters():
        cnt1 =cnt1+1
    ...
    EfficientB1 = to_device(EfficientB1,device)
    optimizer = torch.optim.Adam(EfficientB1.parameters(), lr=lrf,betas=(0.9,0.999))

    optimizer_ft =  torch.optim.Adam(EfficientB1.parameters(), lr=lr)

    train_model(EfficientB1,CTTrain,Criterion,optimizer_ft)

    train_model(EfficientB1,CTTrain,Criterion1,optimizer,epochsf)
    ...
    CTF1,CTAcc,CTSn,CTSp = evaluate_CT(EfficientB1,CTTest)
